        ...
        <div className='container'>
          <div>
            <div className='image-placeholder'>
              <img src={imageUrl} ref={imageRef} onClick={(event) => handleImageClick(event)} />
              {cordinatePickerList.map((item, i) => (
                <div key={i} id={item.hotpsotId} className="hotpsot" style={{ "left": item.clickXPos, "top": item.clickYPos }}></div>
              ))}
            </div>
          </div>
        </div>}
